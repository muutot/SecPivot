...
- 协议核心:`bridge.rs` 实现 `associate`/`test-associate`/`get-logins`/`get-logins-count`/`set-login`/`generate-password`;AES-256-CBC 逐字段加密 + PKCS7;请求 `Verifier` 与响应 `Hmac`(HMAC-SHA256)按 KeePassHttp 语义校验。`set-login` 更新时只改发生变化的用户名/密码(不以当前页面 URL 覆盖主 URL),创建 KeePass 历史快照、标记会话 dirty 并通知桌面刷新(与参考插件的 `CreateBackup` + `UpdateUI` 对齐,不强制立即保存);`generate-password` 返回 20 位随机口令(大小写/数字/符号各至少一位,与应用默认生成器一致)。
...
- **密钥认证(1b)**:`key{username, securityLevel}` → 服务端随机 `sc`,发 `key{sc, securityLevel, features}` → `key{cc, cr}`(cr = SHA256("1"‖secret‖sc‖cc) 小写 hex)→ 校验后发 `key{sr}`(sr = SHA256("0"‖secret‖sc‖cc))。密钥按 username 存会话内(与 bridge_keys 同理,不落 config.json;锁库时是否销毁由 `rpc.keepSessionAfterLock` 决定——默认开启时锁库保留、解锁后扩展无需重新授权,官方 KeePassRPC 行为;扩展侧认证失败自动回退 SRP 重新授权)。
...
- **安全约束(与 Phase 1 一致)**:仅 loopback;锁定时直接错误信封、不自动解锁;旁路密码只在桌面端展示、不入日志;SRP 中间态(K、密钥)存会话内,锁库时按 `rpc.keepSessionAfterLock` 决定是否保留(默认开启,官方 KeePassRPC 行为;此时仅 SRP 会话密钥保留,主密钥/密钥文件/KeePassHttp `bridge_keys` 仍清空);超时 120 s。
...
- **持久化与并发语义**:`AddLogin`/`UpdateLogin` 的协议参数只解析一次；服务端捕获当时的 backend-active `sessionId`，在短锁内对数据库克隆应用操作，然后完全锁外执行 KDF、序列化以及本地/远程写入，最后短锁内完成到原会话。所有会写入 vault 或本地镜像的运行时事务（创建、远程打开/刷新镜像、普通保存、重加密、改密钥、另存为、远程合并、favicon 自动保存、RPC）共享进程级持久化门禁，从 prepare 到 complete 串行，但不占用会话锁，避免两个陈旧快照都成功却后写覆盖前写。写入失败不修改 live session；标签切换不会把结果写入新活动库。若持久化期间另有 UI 编辑，RPC 操作会重放到当前库并保持 dirty，下一次保存会包含两者；同条目竞争按“后完成的 RPC 字段胜出”，UI 中间态保留在条目历史。若原标签关闭或并发删除使完成阶段无法重放，磁盘写入仍是成功事实，响应使用持久化快照的 DTO，不返回误导性的失败。
- **测试策略**:`rpc.rs` 增加 URL 合并模式单测(5 模式 × 多 URL 场景)+ `AddLogin`/`UpdateLogin` 调度单测;`vault.rs` 增加写路径测试(新建于根/指定分组、字段映射、URL 合并、历史快照、回收站拒绝、锁定拒绝、持久化失败原子性、标签切换绑定、并发编辑重放与同条目历史语义)。真机 Kee 扩展 E2E 仍不可行(离线),保留 `~` 证据缺口。
- **Phase 2b 交付状态(已交付)**:`rpc.rs` 新增写路径 DTO(`RpcLoginWrite`/`RpcFieldWrite`,serde 精确还原 `uRLs`/`hTTPRealm`/`formFieldList` 键名)、`merge_urls` 纯函数(5 模式 + `MergeInNewURLs` 源主 URL 提升语义)、共享写请求校验、`handle_jsonrpc` 的 `AddLogin`/`UpdateLogin` 分支(镜像插件的 `ArgumentException` 预校验与 `-32001`/`-32002` 错误码)与特性 `KPRPC_FEATURE_ENTRY_URL_REPLACEMENT`(Kee 将发 `urlMergeMode=5`);`VaultSession` RpcHost 写路径实现 `setPwEntryFromEntry` 字段映射(首 `FFTpassword`→Password 受保护、`FFTusername`→UserName 后者胜、余字段→`displayName` 回退 `name` 的自定义字符串、URL 空格拼接与读路径一致)、父分组解析(空/无效/回收站内 → 根回退)、更新走 `edit_tracking` 历史快照(插件 `CreateBackup` 等价)、回收站条目拒绝(插件允许,但 Kee 读路径永不可见,属防御纵深)，并以 prepare/persist/complete 保证锁外耐久化与失败原子性。**已记录偏差**:附加字段存 KDBX 字符串而非插件私有「KPRPC JSON」配置;更新覆盖字段但不删除陈旧自定义字段(避免误删应用管理字段)。
