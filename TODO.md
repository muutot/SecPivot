...
- [~] Phase 2b (post) — KeePassRPC 写路径(AddLogin/UpdateLogin)、BROWSER_SETTINGS_SYNC 评估——**已立项**:规格已从 KeePassRPC 1.12 插件源码(`KeePassRPCService.JSONRPC.cs`/`KeePassRPCService.cs` `MergeEntries`/`MergeInNewURLs`/`KeePassRPCService.DTOV1.cs` `setPwEntryFromEntry`)与 Kee 4.0.7 扩展源码提取(`docs/browser-integration.md` Phase 2b 细则)。实施顺序:rpc.rs URL 合并纯函数(5 模式)+ `AddLogin`/`UpdateLogin` 调度与 DTO → vault.rs 写路径(复用 Phase 1 `set_login`/`create_login` 模式,含历史快照)→ 特性 `KPRPC_FEATURE_ENTRY_URL_REPLACEMENT` → 测试与文档。(BROWSER_SETTINGS_SYNC 结论:仅客户端特性标志,桌面 loopback 无设置同步通道,不实现不宣告;Kee Vault 事件会话与 v2 方法族超出桌面范围,不实现)
