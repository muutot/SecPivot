...
- Builds **64-bit split-per-ABI signed release Android APKs** (`aarch64`, `x86_64`) in parallel on `ubuntu-latest` (`android` job, `needs: verify`) — the same `CARGO_PROFILE_RELEASE_*` extreme-optimization overrides as the Windows build apply (`target-cpu=x86-64-v3` is x86-64 only and is not set here), avoiding a needlessly large unoptimized native library. The whole network stack is rustls (no `openssl-sys`/`openssl-src` cross-compilation; the historical `TARGET_RANLIB` export is moot but kept for reference), all signing secrets are mandatory, each per-ABI APK (cargo-mobile2 writes `app/build/outputs/apk/<arch>/release/app-<arch>-release.apk`, so the ABI is the arch flavor two levels up, mapped `arm64`→`arm64-v8a`) is staged as `SecPivot-<version>-android-<abi>.apk`, `apksigner verify` plus a `lib/<abi>/` content check gates every APK, and staging fails closed unless both the `arm64-v8a` and `x86_64` assets exist. The verified APKs cross jobs as one-day Actions artifacts, and `publish-android` waits for both builders before uploading them to the draft release, so a slow Windows LTO build cannot trip a fixed polling timeout; missing APKs/release or failed uploads still fail closed
