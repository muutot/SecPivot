...
- Toolbar row (the top chrome, itself the drag region via `data-tauri-drag-region`, frame-less window) owns compact action buttons on the left (new entry · save / save-as · lock, grouped by a 1px `--border-subtle` divider), a **centered compact search box** (`--input-bg`, subtle border, muted placeholder) in the middle, and on the right the per-item visible secondary actions (each `toolbarItems` entry controls whether that action shows directly on the toolbar or is collected into the More menu; window controls — minimize — / maximize-restore □ / close × share `WindowControls.svelte` with per-button visibility `windowMinimize/windowMaximize/windowClose`, close hover = `--danger-color`). The General → Toolbar secondary group lets every secondary action plus the three window buttons be toggled individually; old configs without `toolbarItems` derive initial values from the legacy `toolbarOverflowMenu` platform default. Search is intentionally low-emphasis: this is a password manager, so it must not dominate the header. The vault file name lives in the status bar, not the toolbar.
