...
- Toolbar row (the top chrome, itself the drag region via `data-tauri-drag-region`, frame-less window) owns compact action buttons on the left (new entry / new group · save / save-as · lock, grouped by a 1px `--border-subtle` divider), a **centered compact search box** (`--input-bg`, subtle border, muted placeholder) in the middle, and on the right the view/tool icons (detail toggle, security report, export CSV, settings) then a divider then the window controls (minimize / maximize-restore / close — shared `WindowControls.svelte`, close hover = `--danger-color`). Search is intentionally low-emphasis: this is a password manager, so it must not dominate the header. The vault file name lives in the status bar, not the toolbar.
...
- **Narrow / mobile layout (`@media (max-width: 720px)`):** the app-shell drops its `min-width` floor and becomes a single-column stack. A `menu` toggle in `toolbar-left` opens the group tree as a left slide-in drawer (`.mobile-nav-open` on the shell + a `.mobile-drawer-backdrop` to dismiss; choosing a group closes it). The detail pane becomes a full-width overlay with a `back` button in `EntryDetail` (`onback` prop) returning to the list. Resize handles, the group/entry grid columns, and window controls are hidden. Toolbar text labels are hidden so controls stay icon-only on narrow screens. The entry table still scrolls horizontally as one unit when its fixed column tracks exceed the viewport.
