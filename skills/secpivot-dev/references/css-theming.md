...
- Toolbar row (the top chrome, itself the drag region via `data-tauri-drag-region`, frame-less window) owns left/right action groups (all 18 toolbar buttons `newEntry/save/saveAs/lock` + right group `toggleDetail/securityReport/similarPasswords/hibpCheck/expiredEntries/clearHistory/importMenu/exportMenu/dbSettings/appSettings/moreMenu` + window `windowMinimize/Maximize/Close` are ordered globally via `toolbarFullOrder` 18, assigned per-button sides via `toolbarSides` left/right, and per-item dividers via `toolbarFullSeparators` → `toolbar-divider` within same side; each `toolbarItems` entry (now 15 + 3 window) controls whether that action shows directly on the toolbar or is collected into the More menu / hidden for window buttons, `moreMenu` always visible and sortable; detail toggle uses `eye/eye-off` and is bound to `showDetailOnSelect` (click entry auto-shows detail, `selectionVersion` re-triggers same entry after close, `eye` enabled / `eye-off` disabled), close hover = `--danger-color`). The General → Toolbar secondary group provides the full 18-item sortable list with left/right switches and `|` dividers; old configs without `toolbarItems` derive initial values from the legacy `toolbarOverflowMenu` platform default, empty `toolbarFullOrder` restores default full order. Search is intentionally low-emphasis: this is a password manager, so it must not dominate the header. The vault file name lives in the status bar, not the toolbar.
