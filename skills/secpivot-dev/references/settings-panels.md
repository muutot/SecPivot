...
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| General          | Appearance / Display / Layout / Toolbar / Network → `GeneralSettingsPanel` with `section` prop. The Toolbar tab is the dedicated secondary group for full toolbar收纳: it lists all 18 toolbar buttons (`newEntry/save/saveAs/lock` + right group + `moreMenu` + `windowMinimize/Maximize/Close`), each toggle controls whether the item shows directly on the main toolbar/window chrome (`true`) or is collected into the More menu / hidden (`false`, `moreMenu` always visible), plus global ordering (`toolbarFullOrder`), side assignment (`toolbarSides` left/right) and dividers (`toolbarFullSeparators`), and the detail auto-show toggle (`showDetailOnSelect`). Migration from the legacy `toolbarOverflowMenu` derives the initial `toolbarItems` values; empty `toolbarFullOrder` restores default full order. |
...
The General → Appearance tab owns theme mode (dark/light/custom segmented control). A second segmented control owns the UI language (中文/English, `general.language`, applies live through the `src/lib/i18n/` dictionaries). In custom mode the single `自定义配色` card owns both the live palette and the multi-theme actions — there is no separate `多主题配置` card: the card header merges the theme `Select` (当前配色 vs. a saved `customThemes` entry — edits write `themeColors` live or the selected theme's stored copy), `另存为` (ModalShell prompt, default `自定义 N`, deduped, `添加`), `修改名称` (rename prompt), `应用` (copy into `themeColors` + force `custom`) and `删除`, together with the `深色预设`/`浅色预设`/`恢复默认` preset fills (which fill whichever palette the selector points at). All naming goes through the dialogs; no inline name field is rendered. Dark/light modes show a read-only palette card instead. Switching the mode control to dark/light never mutates the persisted `themeColors`: `selectThemeColors` serves the built-in presets directly for non-custom modes, so returning to custom restores the last custom palette (live edits or the last `应用`d theme) rather than the mode preset.

The General → Layout section owns the always-on density controls: the entry-table row height (`entryRowHeight`, applied via the `--entry-row-height` variable so CSS matches the setting; narrow rows floor at 36px) plus group-tree spacing (gap, padding, indent, radius). Subgroup separator lines are split across two homes: the Display toggle (`showGroupSeparators`, default on) and the Appearance color card (`groupSeparatorColor` hex, empty = theme muted text; the hex field only persists empty/valid values and warns otherwise). There is no compact-mode toggle — the layout settings apply directly. The General → Toolbar secondary group owns full toolbar customization: global ordering (`toolbarFullOrder` 18 ids including `newEntry/save/saveAs/lock` + right group + `moreMenu` + `windowMinimize/Maximize/Close`, drag/Up/Down reorders), per-item dividers (`toolbarFullSeparators` → `toolbar-divider` after the item within same side), per-button side assignment (`toolbarSides` `left`/`right`), per-item visibility (`toolbarItems` 18 toggles, `moreMenu` always visible and sortable, window buttons hide when false) and the detail auto-show toggle (`showDetailOnSelect` `eye/eye-off`, click entry auto-shows detail). Legacy `toolbarOrder/Separators` (right-only 10) remain for migration and are superseded by the full set. The panel owns full-toolbar customization收纳: per-item `toolbarItems` toggles (saveAs, toggleDetail, securityReport, similarPasswords, hibpCheck, importMenu, exportMenu, expiredEntries, clearHistory, dbSettings, appSettings plus windowMinimize/windowMaximize/windowClose — `— □ ×`). Each toggle controls whether that action shows directly on the main toolbar/window chrome (`true`) or is collected into the More menu / hidden for window buttons (`false`). `toolbarItems` defaults follow the legacy `toolbarOverflowMenu` platform default (desktop: saveAs/toggleDetail/securityReport/exportMenu/appSettings `true`, others `false`; mobile all toolbar actions `false`; window buttons all `true`). Old configs without `toolbarItems` derive initial values from the legacy `toolbarOverflowMenu` flag during normalization.
