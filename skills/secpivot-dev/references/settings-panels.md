...
The General → Appearance tab owns theme mode (dark/light/custom segmented control). In custom mode the 自定义配色 card edits the live palette's semantic colors and carries the 多主题配置 block (an extension of the custom palette, not a separate surface) placed as the first row directly beneath the 自定义配色 header and above the color grid: a Select switches the shared color grid between 当前配色 (edits write `themeColors` live) and a saved `customThemes` entry (edits write only that theme's stored copy), 另存为 opens a `ModalShell` prompt for the new scheme name (default `自定义 N`, deduped, `添加` confirms) and snapshots the displayed palette, 修改名称 opens a rename prompt for the selected theme, 应用 copies the selected theme into `themeColors` (and forces theme `custom`), 删除 removes it; 深色/浅色预设 and 恢复默认 fill whichever palette the selector currently points at. No inline name field is rendered — all naming goes through the dialogs. Dark/light modes show a read-only palette card instead.
