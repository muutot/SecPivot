...
- Three-pane main layout: group tree (left) → entry list (center) → detail (right). Rows are transparent at rest, use `--hover-bg` on hover/focus, and mix selection color into the selected state. The detail pane is hideable via a chevron toggle in the toolbar (hidden collapses `--detail-width` to `0` and unmounts the pane).
