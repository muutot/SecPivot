...
| `remote`   | `RemoteSettings`   | `endpoint` (default `https://s3.amazonaws.com`), `region` (`us-east-1`), `bucket`, `accessKey`, `secretKey` (plaintext by design, see `security-model.md`), `prefix`, `localDir` (default `remote`), `backupCount` (0–10, default 3). Normalized on both sides: trimmed strings, empty `localDir` → `remote`, `backupCount` clamped.               |
