...
- Screen-capture guard: while a vault is open the main window sets `SetWindowDisplayAffinity(WDA_MONITOR)` (Windows only) so screenshots, recordings and screen sharing render it as a solid block; the guard is released on lock/close. TCATO overlay windows and the lock screen are not guarded (no secrets rendered there).
