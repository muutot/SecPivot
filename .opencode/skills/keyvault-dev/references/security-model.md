...
- **S3 remote vaults (opt-in)**: access keys are stored **plaintext in `config.json`** by design (user-confirmed) — they are a secondary credential, never a vault master password; anyone who reads the config can read/write objects in that bucket but cannot decrypt any KDBX. Risk mitigations: keys never appear in logs, each command rebuilds the client from the frontend config (no caching), transport uses HTTPS (or the user's configured MinIO endpoint), and the payload is already encrypted KDBX. Local mirror copies ("保存到本地" mode) live under `<app_data>/Storage/remote/<sanitized>/` at rest with the same KDBX encryption. Threat model note: S3 access keys in a plaintext config are only as safe as the local filesystem; if that is an unacceptable trade-off, the user should not enable the remote feature.
