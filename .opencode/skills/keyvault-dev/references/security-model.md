...
- Screen-capture guard: while a vault is open the main window sets `SetWindowDisplayAffinity(WDA_EXCLUDEFROMCAPTURE)` (Windows 10 2004+; Windows only) so screenshots, recordings and screen sharing omit it, while the window stays fully visible on screen. `WDA_MONITOR` must never be used here — it renders the window as a solid black box on the physical display (black-screen bug). The guard is released on lock/close. TCATO overlay windows and the lock screen are not guarded (no secrets rendered there).
