...
| `src-tauri/src/vault/mod.rs` | KeePass session: open/create/close/get_state/save, entry & group CRUD, serialization, auto-type match scoring, recycle-bin helpers (`recycle_bin_id`/`parse_entry_id` exposed to `vault::hosts`)                                                                                                                             |
...
- `src-tauri/src/bridge.rs` + `src-tauri/src/bridge_server.rs` + `src-tauri/src/rpc.rs` + `src-tauri/src/rpc_server.rs` + `src-tauri/src/vault/hosts.rs` (protocol ↔ server ↔ session boundaries)
- `TODO.md`
